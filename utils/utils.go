...
func JSONEncodePretty(value interface{}) []byte {
...
func Print(arg ...interface{}) {
	for _, a := range arg {
		os.Stdout.WriteString(fmt.Sprintf("%v", a))
	}
}
...
func TimeFormat(format string, now ...time.Time) string {
	var t1 time.Time
	if len(now) > 0 {
		t1 = now[0]
	} else {
